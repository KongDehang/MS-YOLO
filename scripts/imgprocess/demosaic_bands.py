...
def demosaic_specific_bands(raw_img, band_matrix):
    ...
    for band_idx, wavelength in enumerate(band_list):
        ...
        np.argwhere(np.array(band_matrix) == wavelength)  # 形如[[i,j], [i,j], ...]
...
def fast_demosaic(raw_img, band_matrix):
    ...
    for band_idx, b in enumerate(bands):
        ...
        small_img = cv2.resize(
            band_img, (small_w, small_h), interpolation=cv2.INTER_NEAREST
        )  #  nearest确保有效像素不变

        # 步骤2：上采样回原始尺寸（用线性插值填充空白）
        band_img = cv2.resize(small_img, (w, h), interpolation=cv2.INTER_LINEAR)  # 线性插值恢复细节
