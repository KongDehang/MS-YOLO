...
class ConfusionMatrix(DataExportMixin):
    ...
    def process_batch(
        ...
        if gt_cls.shape[0] == 0:  # Check if labels is empty
            if not no_pred:
                ...
                for i, dc in enumerate(detection_classes):
                    # Skip if detection class is out of bounds (for multi-head models)
                    if dc < self.nc:
        ...
        if no_pred:
            ...
            for i, gc in enumerate(gt_classes):
                # Skip if ground truth class is out of bounds (for multi-head models)
                if gc < self.nc:
    ...
    @plt_settings()
    def plot(self, normalize: bool = True, save_dir: str = "", on_plot=None, names_suffix: str = ""):
        """
        ...
        Args:
            ...
            names_suffix (str, optional): Suffix to add to the filename (e.g., "_cls2").
        """
        ...
        plot_fname = Path(save_dir) / f"{title.lower().replace(' ', '_')}{names_suffix}.png"
