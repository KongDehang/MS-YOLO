...
def detect_image_size(total_pixels: int) -> Optional[tuple[int, int]]:
    """
    Attempt to detect image dimensions from total pixel count. Prioritizes square-like dimensions.
...
def read_raw(filename: str | Path) -> Optional[np.ndarray]:
    """
    Read a raw image file with auto-detected dimensions. Assumes uint8 data type and a 4-byte integer header containing
    data length.
