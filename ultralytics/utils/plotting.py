...
def plot_images(
    ...
    cls2 = labels.get("cls2", None)  # Secondary classification head
            ...
            if len(bboxes):
                ...
                for j, box in enumerate(boxes.astype(np.int64).tolist()):
                    ...
                    if labels or conf[j] > conf_thres:
                        ...
                        # Add cls2 label if available
                        if cls2 is not None and len(cls2) > 0:
                            idx_cls2 = batch_idx == i
                            if idx_cls2.sum() > 0 and j < len(cls2[idx_cls2]):
                                c2 = int(cls2[idx_cls2][j])
                                c2_name = names2.get(c2, c2) if names2 else c2
                                label = f"{label} | {c2_name}"
