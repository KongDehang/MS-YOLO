...
class Detect(nn.Module):
    ...
    def __init__(self, nc: int = 80, ch: tuple = ()):
        ...
        self.nc2 = 0  # optional secondary class count, may be set later
        self.nl = len(ch)  # number of detection layers
        self.reg_max = 16  # DFL channels (ch[0] // 16 to scale 4/8/12/16/20 for n/s/m/l/x)
        self.no = nc + self.reg_max * 4  # number of outputs per anchor (nc2 will be added later if present)
        ...
        # optional secondary classification head (created later if nc2 > 0)
        self.cv3b = None
        self.cv3b_transform = None  # Independent feature transform for secondary classification
    ...
    def forward(self, x: list[torch.Tensor]) -> list[torch.Tensor] | tuple:
        ...
        for i in range(self.nl):
            ...
            if hasattr(self, "cv3b") and self.cv3b is not None:
                # Use independent feature transform before cv3b
                if hasattr(self, "cv3b_transform") and self.cv3b_transform is not None:
                    x_transformed = self.cv3b_transform[i](x[i])
                    parts.append(self.cv3b[i](x_transformed))
                else:
                    # Fallback to original behavior if transform not available
                    parts.append(self.cv3b[i](x[i]))
    ...
    def _inference(self, x: list[torch.Tensor]) -> torch.Tensor:
        ...
        if self.export and self.format in {"saved_model", "pb", "tflite", "edgetpu", "tfjs"}:  # avoid TF FlexSplitV ops
            ...
            if getattr(self, "nc2", 0):
                box, cls, cls2 = x_cat.split((self.reg_max * 4, self.nc, self.nc2), 1)
            else:
        ...
        if getattr(self, "nc2", 0):
            return torch.cat((dbox, cls.sigmoid(), cls2.sigmoid()), 1)
    ...
    def bias_init(self):
        ...
        for idx, (a, b, s) in enumerate(zip(m.cv2, m.cv3, m.stride)):  # from
            a[-1].bias.data[:] = 1.0  # box
            b[-1].bias.data[: m.nc] = math.log(5 / m.nc / (640 / s) ** 2)  # cls (.01 objects, 80 classes, 640 img)
            # initialize secondary head bias if present
            if getattr(m, "cv3b", None) is not None:
                b3 = m.cv3b[idx]
                # set bias for secondary classes similarly (if any)
                if hasattr(b3[-1], "bias"):
                    b3[-1].bias.data[: m.nc2] = math.log(5 / max(1, m.nc2) / (640 / s) ** 2)
        if self.end2end:
