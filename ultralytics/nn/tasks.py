...
class BaseModel(torch.nn.Module):
    ...
    def load(self, weights, verbose=True):
        ...
        # custom head modules (like cv3b) when checkpoint doesn't contain them.
        state_dict = self.state_dict()

        # conservative merge: build a merged dict starting from state_dict (current model) and update from csd
        merged = {k: v.clone() for k, v in state_dict.items()}
        copied = 0
        for k, v in csd.items():
            if k not in merged:
                # checkpoint has extra keys (e.g., extra heads) - skip
                continue
            if merged[k].shape == v.shape:
                merged[k] = v.to(merged[k].dtype)
                copied += 1
            else:
                # shapes differ: attempt to partially copy for first_conv handled below, otherwise skip to preserve model
                continue

        # handle first conv partial copy for multi-channel checkpoints
        first_conv = "model.0.conv.weight"  # hard-coded to yolo models for now
        if first_conv in csd and first_conv in merged and merged[first_conv].shape != csd[first_conv].shape:
            c1, c2, h, w = merged[first_conv].shape
            cc1, cc2, ch, cw = csd[first_conv].shape
            if ch == h and cw == w:
                c1_c, c2_c = min(c1, cc1), min(c2, cc2)
                merged[first_conv][:c1_c, :c2_c] = csd[first_conv][:c1_c, :c2_c]
                copied += 1

        # Now load merged state_dict with strict=False so unmatched keys are left intact
        self.load_state_dict(merged, strict=False)
        len_updated_csd = copied
        if verbose:
            LOGGER.info(f"Transferred {len_updated_csd}/{len(state_dict)} items from pretrained weights (safe-merged)")
...
class DetectionModel(BaseModel):
    ...
    def __init__(self, cfg="yolo11n.yaml", ch=3, nc=None, verbose=True):
        ...
        # Support for secondary classification head
        if "nc2" in self.yaml and self.yaml["nc2"] > 0:
            self.names2 = {i: f"{i}" for i in range(self.yaml["nc2"])}  # default names2 dict
        else:
            self.names2 = None
        ...
        if isinstance(m, Detect):  # includes all Detect subclasses like Segment, Pose, OBB, YOLOEDetect, YOLOESegment
            ...
            # Create stride tensors before modifying head architecture
            def _forward(x):
            ...
            self.model.eval()  # Avoid changing batch statistics during stride calc
            m.training = True  # Setting it to True to properly return strides
            m.stride = torch.tensor([s / x.shape[-2] for x in _forward(torch.zeros(1, ch, s, s))])  # forward
            self.stride = m.stride
            self.model.train()  # Set model back to training(default) mode

            # Optional secondary class head (nc2) support: create cv3b if yaml/data specifies nc2
            nc2 = int(self.yaml.get("nc2", 0)) if self.yaml.get("nc2", 0) is not None else 0
            if nc2 > 0:
                # set attribute before bias_init
                m.nc2 = nc2
                try:
                    m.no = int(m.no) + int(nc2)  # update outputs per anchor
                except Exception:
                    pass

                # Build feature transform layers for cv3b (独立特征变换)
                ...
                for i in range(m.nl):
                    ...
                    try:
                        ...
                        if hasattr(first_conv, 'conv'):
                            ...
                            if hasattr(first_conv, 'conv'):
                                ...
                                in_channels = 256  # fallback default
                    ...
                    # P4/P5 (i=1/2) use n=1 for efficiency
                    n_bottlenecks = 2 if i == 1 else 1
                    ...
                    if isinstance(block, nn.Sequential):
                        ...
                        if isinstance(last, nn.Conv2d):
                            ...
                            children[-1] = new_last
                            cv3b_modules.append(nn.Sequential(*children))
                        else:
                            cv3b_modules.append(block)
                    else:
                        try:
                            # Find last Conv2d in module children
                            sub_children = [c for c in block.children()]
                            for j in range(len(sub_children) - 1, -1, -1):
                                if isinstance(sub_children[j], nn.Conv2d):
                                    in_ch = sub_children[j].in_channels
                                    new_last = nn.Conv2d(
                                        in_ch,
                                        nc2,
                                        kernel_size=sub_children[j].kernel_size,
                                        stride=sub_children[j].stride,
                                        padding=sub_children[j].padding,
                                        bias=sub_children[j].bias is not None,
                                    )
                                    sub_children[j] = new_last
                                    cv3b_modules.append(nn.Sequential(*sub_children))
                                    break
                            else:
                                cv3b_modules.append(block)
                        except Exception:
            ...
            # Initialize biases after head architecture is finalized
...
def parse_model(d, ch, verbose=True):
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        if m in base_modules:
            ...
            c1 = ch[f]
            args = [*args[1:]]
        elif m in frozenset({BandAttention, BandAttentionV2}):
            # BandAttention: auto-detect input channels from previous layer
            c1 = ch[f]
            c2 = c1  # BandAttention doesn't change channel count
