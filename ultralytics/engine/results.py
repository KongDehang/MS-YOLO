...
class Results(SimpleClass, DataExportMixin):
    ...
    def __init__(
        ...
        self.names = names
        # secondary-class (per-box) labels and names
        self.names2 = names2
        # cls2 is expected to be a 1D tensor/array with length == number of boxes (optional)
        self.cls2 = cls2
        # cls2_probs stores the secondary-class probability vectors (N x nc2) when available
        self.cls2_probs = cls2_probs
        self.path = path
        self.save_dir = None
    ...
    def plot(
        ...
        if pred_boxes is not None and show_boxes:
            ...
            for i, d in enumerate(pred_list):
                ...
                # include secondary-class name if available (self.cls2, self.names2)
                secondary = None
                if getattr(self, "cls2", None) is not None and getattr(self, "names2", None) is not None:
                    try:
                        if i < len(self.cls2):
                            c2 = int(self.cls2[i])
                            secondary = self.names2.get(c2, c2) if self.names2 else c2
                    except Exception:
                        secondary = None

                # build label showing primary name+score and optional secondary name+score
                if secondary is not None:
                    ...
                    if getattr(self, "cls2_probs", None) is not None:
                        try:
                            ...
                            sec_score = None
                    secondary_part = f"{secondary} {sec_score:.2f}" if sec_score is not None else f"{secondary}"
                    label = f"{primary_part} | {secondary_part}" if labels else None
                else:
    ...
    def verbose(self) -> str:
        ...
        if boxes:
            # Build a per-detection listing showing primary name+score and optional secondary name+score
            out = []
            pred_list = list(boxes)
            for i, d in enumerate(pred_list):
                try:
                    c = int(d.cls)
                    conf = float(d.conf)
                except Exception:
                    c, conf = int(d[5]) if len(d) > 5 else 0, float(d[4]) if len(d) > 4 else 0.0
                primary = f"{self.names[c]} {conf:.2f}"
                ...
                if secondary is not None:
                    if sec_score is not None:
                        out.append(f"{primary} | {secondary} {sec_score:.2f}")
                    else:
