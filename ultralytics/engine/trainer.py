...
class BaseTrainer:
    ...
    def set_model_attributes(self):
        ...
        # Set names2 if available in data config (for multi-head models)
        if "names2" in self.data:
            self.model.names2 = self.data["names2"]
        ...
        for f in self.last, self.best:
            if f.exists():
                if f is self.last:
                    ...
                    # Pass trainer=None to explicitly set validator.training=False in __call__()
                    # This ensures validator returns full stats dict instead of simplified training dict
                    self.metrics = self.validator(trainer=None, model=f)
                    # Don't remove fitness - keep combined fitness and individual cls1/cls2 fitness
                    # self.metrics.pop("fitness", None)  # Commented out to preserve fitness info
                    self.run_callbacks("on_fit_epoch_end")
