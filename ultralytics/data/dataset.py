...
class YOLODataset(BaseDataset):
    ...
    def cache_labels(self, path: Path = Path("./labels.cache")) -> dict:
        ...
        with ThreadPool(NUM_THREADS) as pool:
            ...
            for im_file, lb, shape, segments, keypoint, cls2, nm_f, nf_f, ne_f, nc_f, msg in pbar:
                nm += nm_f
                nf += nf_f
                ne += ne_f
                nc += nc_f
                if im_file:
                    ...
                    if cls2 is not None:
                        # ensure shape (n,1)
                        label_entry["cls2"] = np.array(cls2, dtype=np.float32).reshape(-1, 1)
    ...
    def collate_fn(batch: list[dict]) -> dict:
        ...
        for i, k in enumerate(keys):
            ...
            if k in {"masks", "keypoints", "bboxes", "cls", "cls2", "segments", "obb"}:
                # Convert numpy arrays to tensors before concatenation
