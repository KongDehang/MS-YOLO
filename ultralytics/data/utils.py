...
IMG_FORMATS = {"bmp", "dng", "jpeg", "jpg", "mpo", "png", "tif", "tiff", "webp", "pfm", "heic", "raw"}  # image suffixes
...
def verify_image_label(args: tuple) -> list:
    ...
    or the standard 5-column YOLO format:
        cls x_center y_center width height
    """
    im_file, lb_file, prefix, keypoint, num_cls, num_cls2, nkpt, ndim, single_cls = args
    ...
    try:
        ...
        if ext == "raw":
            ...
            # Verify images with PIL for supported formats
            im = Image.open(im_file)
            im.verify()  # PIL verify
            shape = exif_size(im)  # image size
            shape = (shape[1], shape[0])  # hw
        ...
        if os.path.isfile(lb_file):
            ...
            with open(lb_file, encoding="utf-8") as f:
                ...
                # segment-style labels (longer than 6 columns) -> convert to boxes
            ...
            if nl := len(lb):
                if keypoint:
                    ...
                    # Support either 5-column YOLO (cls x y w h) or 6-column multi-task (cls1 cls2 x y w h)
                    if lb.shape[1] == 6:
                        # parse cls1 and cls2
                        cls2 = lb[:, 1:2].astype(np.float32)
                        # keep lb in 5-col format where first col is cls1 and remaining 4 are bbox
                        lb = np.concatenate((lb[:, 0:1], lb[:, 2:6]), axis=1)
                    else:
                ...
                # All labels
                max_cls = 0 if single_cls else int(lb[:, 0].max())  # max label count for primary class
                assert max_cls < num_cls, (
                    f"Label class {int(max_cls)} exceeds dataset class count {num_cls}. "
                    f"Possible class labels are 0-{num_cls - 1}"
                )
                # Verify secondary class ids if present
                if cls2 is not None:
                    max_cls2 = int(cls2.max()) if cls2.size else 0
                    # when names2 not provided num_cls2 can be 0 -> skip check
                    if num_cls2:
                        assert max_cls2 < num_cls2, (
                            f"Secondary label class {int(max_cls2)} exceeds dataset class count {num_cls2}. "
                            f"Possible class labels are 0-{num_cls2 - 1}"
                        )
        ...
        lb = lb[:, :5]
        return im_file, lb, shape, segments, keypoints, cls2, nm, nf, ne, nc, msg
    except Exception as e:
        nc = 1
        msg = f"{prefix}{im_file}: ignoring corrupt image/label: {e}"
